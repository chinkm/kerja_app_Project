...
export default function VoiceProfileModule() {
  ...
  const handleStartListening = () => {
    if (generationCount >= MAX_FREE_GENERATIONS) {
      setError(
        `You have reached the free generation limit of ${MAX_FREE_GENERATIONS}. Please upgrade to continue.`,
      );
      return;
    }
  ...
  const handleGenerateProfile = async () => {
    ...
    if (!apiKey) {
      setError(
        "Error: System Gemini API Key is missing. Check your local .env configuration file.",
      );
      return;
    }
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 font-sans">
      ...
      <div className="mb-6 text-sm font-medium text-gray-600">
        ...
        <span className="font-bold">{MAX_FREE_GENERATIONS}</span> polished
        profiles. You have used{" "}
      ...
      <div className="mb-6">
        ...
        <select
          ...
          onChange={(e) =>
            setSelectedLang(
              SUPPORTED_LANGUAGES.find((lang) => lang.name === e.target.value),
            )
          }
      ...
      <div className="flex gap-4 items-center mb-6">
        ...
        >
          {listening
            ? "⏹ Stop Recording"
            : `🎤 Start Speaking in ${selectedLang.name}`}{" "}
        </button>
      ...
      {isLimitReached ? (
        <div className="p-6 bg-amber-50 border border-amber-300 rounded-md text-center mb-6">
          ...
          <p className="text-amber-700 text-sm mb-4">
            You have used all 3 free profile generations. Please subscribe to
            continue optimizing your professional profile!
          </p>
          <button
            onClick={() =>
              alert("Redirecting to payment/subscription plan screen...")
            }
      ...
      {polishedProfile && (
        <div className="mt-6 p-4 bg-gray-50 border rounded-md min-h-[100px] text-gray-800">
          <h2 className="text-lg font-semibold mb-2 text-gray-700">
            Polished Written Profile (
            {selectedLang.name.includes("Sabahan")
              ? "English Output"
